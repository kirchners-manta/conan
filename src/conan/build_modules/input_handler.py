...
class Parser:
    ...
    def __check_for_invalid_expressions(self, token_list):
        ...
        for operator in operator_list:
            ...
            operand_list.append(operator[0] + 1)
        if (
            min(operand_list) < 0  # first element is an operator
            or max(operand_list) > len(token_list) - 1  # last element is an operator
            or len(operand_list) > len(set(operand_list))
        ):  # two operators share an operand
            return None
        else:
            ...
            for operator in operator_list:
                expression_list.append(
                    {
                        "operand1": token_list[operator[0] - 1],
