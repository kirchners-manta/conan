...
def Coord_number_prep(inputdict):
    ...
    do_xyz_analysis = ddict.get_input('Do you want to perform a 3d analysis of the coordination number? [y/n]', args,
                                      'string')
    ...
    if do_xyz_analysis == 'n':
        ...
        if referencepoint == 'y':
            structure_as_reference = ddict.get_input('Should an existing structure be used as reference point? [y/n]',
                                                     args, 'string')
            if structure_as_reference == 'n':
                ...
                z_referencepoint[1] = ddict.get_input(
                    'Enter the z-values of the reference point point (x- and y- dependent analysis not implemented yet, sorry D:))',
                    args, 'float')
                ...
                if poresonly == 'n':
                    ...
                    if len(CNT_centers) != 0:
                        z_referencepoint[0] = max_z_pore[0] - box_size[
                            2]  # z-coordinate of first pore (outside of the box)
                        ...
                        z_referencepoint[3] = box_size[2] + min_z_pore[
                            0]  # z-coordinate of second pore outside of the box
                    elif len(Walls_positions) != 0:
                        z_referencepoint[0] = Walls_positions[0] - box_size[
                            2]  # z-coordinate of first Wall (outside of the box)
                        ...
                        z_referencepoint[3] = box_size[2] + Walls_positions[
                            1]  # z-coordinate of second Wall outside of the box
    ...
    if referencepoint == 'y':
        ...
        if poresonly == 'n':
            coord_bulk_bin_edges = np.linspace(0, max((z_referencepoint[3] - z_referencepoint[2]) / 2,
                                                      (z_referencepoint[1] - z_referencepoint[0]) / 2),
        ...
        for species_pair in frame_distances[['Species1', 'Species2']].drop_duplicates().values:
            ...
            if 'Distance_to_referencepoint' in frame_distances:
                distances = frame_distances.loc[(frame_distances['Species1'] == species_pair[0]) &
                                                (frame_distances['Species2'] == species_pair[1]), ['Molecule1',
                                                                                                   'Distance',
                                                                                                   'Distance_to_referencepoint']]
            else:
                distances = frame_distances.loc[(frame_distances['Species1'] == species_pair[0]) &
                                                (frame_distances['Species2'] == species_pair[1]), ['Molecule1',
                                                                                                   'Distance']]
...
def Coord_post_processing(inputdict):
    ...
    if 'Zbin' in processed_coord_df:
        processed_coord_df = processed_coord_df.groupby(['Reference_Observable', 'Zbin']).apply(
            weighted_average).reset_index()
    else:
            ...
            for molecule, molecule_distances in grouped_distances:
                ...
                if 'Distance_to_referencepoint' in molecule_distances:
                    ...
                    Zbin = coord_bulk_bin_edges[
                        np.digitize(Distance_to_referencepoint, coord_bulk_bin_edges) - 1].round(2)
