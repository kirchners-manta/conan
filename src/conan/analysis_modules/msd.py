...
def msd_prep(inputdict, traj_file, molecules):
    ...
    # check the id_frame by finding all different numbers in the Species column,
    # where Liquid is written in the Struc column.
    ...
    for species in num_liq_species:
        num_molecules = len(
            first_frame[(first_frame["Struc"] == "Liquid") & (first_frame["Species"] == species)]["Molecule"].unique()
        )
        num_liq_species_dict[species] = num_molecules
        # also store the exact molecule numbers in a list and add them to the dictionary
        molecule_numbers = first_frame[(first_frame["Struc"] == "Liquid") & (first_frame["Species"] == species)][
            "Molecule"
        ].unique()
        ...
        ddict.printLog(
            "New array called displacements_" + str(species) + " created of size " + str(displacements.shape)
        )

    # Set up the initial COM dataframe containing the coordinates of each liquid molecule in the system
    # Loop over all species and all molecules and calculate the COM of each molecule using the
    # calculate_com function in utils.
    ...
    for (species, molecule), molecule_data in first_frame_grouped:
        ...
        molecule_dict = {"Species": species, "Molecule": molecule, "X": com[0], "Y": com[1], "Z": com[2]}
...
def msd_analysis(inputdict, traj_file, molecules):
    ...
    for species in inputdict["num_liq_species_dict"]:
        for molecule in inputdict["species_mol_numbers"][species]:
            COM_initial = COM_frame_reference[
                (COM_frame_reference["Species"] == species) & (COM_frame_reference["Molecule"] == molecule)
            ]
            COM_current = COM_frame_current[
                (COM_frame_current["Species"] == species) & (COM_frame_current["Molecule"] == molecule)
            ]
            displacement = np.array(
                [
                    COM_current["X"].values[0] - COM_initial["X"].values[0],
                    COM_current["Y"].values[0] - COM_initial["Y"].values[0],
                    COM_current["Z"].values[0] - COM_initial["Z"].values[0],
                ]
            )
