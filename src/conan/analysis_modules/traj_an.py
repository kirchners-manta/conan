...
def traj_chunk_info(id_frame):
    ...
    return number_of_frames, number_of_lines_per_chunk, number_of_lines_last_chunk, number_of_chunks, chunk_size, last_chunk_size

# MAIN
def analysis_opt(id_frame, CNT_centers, box_size, tuberadii, min_z_pore, max_z_pore, length_pore, Walls_positions) -> None:
    ...
    if choice == 1:
        ...
        if len(CNT_centers) >= 0:
            trajectory_analysis(id_frame, CNT_centers, box_size, tuberadii, min_z_pore, max_z_pore, length_pore, Walls_positions)
        else:
...
def generating_pictures(id_frame, CNT_centers, box_size) -> None:
    ...
    if analysis1_choice == 1:
        ...
        for i in range(len(CNT_centers)):
            ...
            if add_liquid == 'y':
                add_liquid2 = ddict.get_input('Add all confined atoms (1), or entire molecules (2) ? [1/2] ', args, 'int')
                ...
                add_centerpoint = ddict.get_input(f"Add the center point of the CNT{i+1} to the file? [y/n] ", args, 'string')
                if add_centerpoint == 'y':         
                    CNT_atoms_pic.loc[len(CNT_atoms_pic.index)] = ['X', CNT_centers[0][0], CNT_centers[0][1], CNT_centers[0][2]]      
...
def trajectory_analysis(id_frame, CNT_centers, box_size, tuberadii, min_z_pore, max_z_pore, length_pore, Walls_positions) -> None:
    ...
    spec_atom = []
    ddict.printLog('')
    analysis_spec_molecule = ddict.get_input(f"Do you want to perform the analysis for a specific molecule kind? (y/n) ", args, 'string')
    ...
    number_of_frames, number_of_lines_per_chunk, number_of_lines_last_chunk, number_of_chunks, chunk_size, last_chunk_size = traj_chunk_info(id_frame)
    ...
    if analysis_choice2 == 1 or analysis_choice2 == 2:
        from conan.analysis_modules.rad_dens import \
            raddens_prep as main_loop_preparation
        if analysis_choice2 == 1:
            from conan.analysis_modules.rad_dens import \
                radial_density_analysis as analysis
        if analysis_choice2 == 2:
            from conan.analysis_modules.rad_dens import radial_charge_density_analysis as analysis
        from conan.analysis_modules.rad_dens import \
            raddens_post_processing as post_processing

    if analysis_choice2 == 3:
        from conan.analysis_modules.axial_dens import \
            accessible_volume_analysis as analysis
        from conan.analysis_modules.axial_dens import \
            accessible_volume_prep as main_loop_preparation
        from conan.analysis_modules.axial_dens import \
            accessible_volume_processing as post_processing

    if  analysis_choice2 == 4:
        from conan.analysis_modules.axial_dens import \
            axial_density_analysis as analysis
        from conan.analysis_modules.axial_dens import \
            axial_density_prep as main_loop_preparation
        from conan.analysis_modules.axial_dens import \
            axial_density_processing as post_processing

    if analysis_choice2 == 5:
        from conan.analysis_modules.coordination_number import \
            Coord_chunk_processing as chunk_processing
        from conan.analysis_modules.coordination_number import \
            Coord_number_analysis as analysis
        from conan.analysis_modules.coordination_number import \
            Coord_number_prep as main_loop_preparation
        from conan.analysis_modules.coordination_number import \
            Coord_post_processing as post_processing
    ...
    if analysis_choice2 == 5:
        if maindict['do_xyz_analysis'] == 'y':
            from coordination_number import \
                Coord_number_xyz_analysis as analysis
            from coordination_number import \
                Coord_xyz_chunk_processing as chunk_processing
            from coordination_number import \
                Coord_xyz_post_processing as post_processing
    ...
    maindict['regional'] = ddict.get_input('Do you want the calculation to be performed in a specific region? [y/n] ', args, 'string')
    regions = [0]*6
