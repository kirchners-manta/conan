...
def accessible_volume_processing(inputdict):
    ...
    if pore_disp_atom == 'y':
        ...
        f.write('%s\t%0.3f\t%0.3f\t%0.3f\n' % (
        maxdisp_atom_row['Atom'], float(maxdisp_atom_row['X']), float(maxdisp_atom_row['Y']),
...
def axial_density_prep(inputdict):
    ...
    num_increments = int(
        ddict.get_input('How many increments per section do you want to use to calculate the density profile? ', args,
                        'int'))
    ddict.printLog(
...
def axial_density_analysis(inputdict):
    ...
    zdens_df_temp = split_frame.groupby(pd.cut(split_frame['Z'].astype(float), z_bin_edges))['Mass'].sum().reset_index(
        name='Weighted_counts')
    # Calculate the number of atoms in each bin (no weighting).
    zdens_df_temp['Counts'] = \
    split_frame.groupby(pd.cut(split_frame['Z'].astype(float), z_bin_edges))['Mass'].count().reset_index(name='Counts')[
        'Counts']
...
def axial_density_processing(inputdict):
    ...
    if mirror_dens_CNT == 'y':
        ...
        for i in range(int(len(inside_CNT))):
            inside_CNT_df['Density [u/Ang^3]'][inside_CNT[i]] = ((results_zd_df['Density [u/Ang^3]'][inside_CNT[i]] +
                                                                  results_zd_df['Density [u/Ang^3]'][
                                                                      inside_CNT[-i - 1]]) / 2).copy()
    ...
    if zdens_data_plot == 'y':
        fig, ax = plt.subplots()
        ax.plot(results_zd_df['Bin_center'], results_zd_df['Density [g/cm^3]'], '-', label='Axial density function',
                color='black')
