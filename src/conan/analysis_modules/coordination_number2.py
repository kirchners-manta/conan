...
def coordination_number_analysis(traj_file, molecules, an):
    ...
    coordination_number.axial_density_processing()
...
def Coord_number_analysis(inputdict, traj_file, molecules):
    ...
    # Now we have to calculate the distance between the COM of each molecule in the mol_com_reference and the COM of all
    ...
    if referencepoint == "y":
        ...
        distance_to_referencepoint = np.abs(mol_com_reference["Z_COM"].values[:, np.newaxis] - z_referencepoint).min(
            axis=1
        )
...
def mirror_PBC(box_size, coord_dist, mol_com):
    ...
    for dim in range(3):
        ...
        # Take care of edges. The if conditions are there to prevent molecules from getting mirrored twice (This is also
        # the only reason
        ...
        for i in range(3):
            if i != dim:
                for j in range(i + 1, 3):
                    if j != dim:
                        # Calculate distances to box boundaries. For edges we have to do this for two dimensions at the
                        # same time
                        ...
                        mirrored_max_edge = mol_com.loc[close_to_max_edge_i & close_to_max_edge_j].copy()
                        ...
                        mirrored_min_edge = mol_com.loc[close_to_min_edge_i & close_to_min_edge_j].copy()
                        ...
                        mirrored_min_max_edge = mol_com.loc[close_to_min_edge_i & close_to_max_edge_j].copy()
                        ...
                        mirrored_max_min_edge = mol_com.loc[close_to_max_edge_i & close_to_min_edge_j].copy()
    ...
    for i in range(3):
        for j in range(i + 1, 3):
            for k in range(j + 1, 3):
                ...
                # New coordinates for all molecuels in close proximity are calculated and appended to
                # mirrored_max_corner
...
def Coord_post_processing(inputdict):
    ...
    # If the column 'Zbin' does not exist in 'processed_coord_df' we dropped it earlier as the user chose to average the
    ...
    for species_pair in avg_coord_df["Reference_Observable"].unique():
        ...
        # if the column 'Zbin' exists in the processed data (meaning the user wants to see the coordination number in
...
def Coord_xyz_chunk_processing(inputdict):
    ...
    # Set up a DataFrame for the results
    coord_df = pd.DataFrame(
        columns=["Frame", "Reference", "Observable", "Molecule", "Xbin", "Ybin", "Zbin"] + list(coord_bin_edges[:-1])
    )
...
def Coord_xyz_post_processing(inputdict):
    ...
    for species_pair in plot_df["Reference_Observable"].unique():
        ...
        with open(cube_filename, "w") as f:
            ...
            # The next three lines give the number of bins along each axis (x, y, z) followed by the axis vector
            # (basically size of the bins).
            # We need to use e.g. len(xbin_edges) - 1 because xbin_edges is just the number of bin EDGES, not the number
            # of BINS.
