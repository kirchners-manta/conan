    ...
    if analysis_choice2 == 2:
        ...
        if id_frame['Charge'].isnull().values.all():
            ...
            if charge_add == 'y':
                ...
                id_frame['Struc_Species_Label'] = id_frame['Struc'].astype(str) + '_' + id_frame['Species'].astype(
                    str) + '_' + id_frame['Label'].astype(str)
...
def radial_density_analysis(inputdict):
    ...
    raddens_df_temp = split_frame.groupby(pd.cut(split_frame['Distance'], raddens_bin_edges))['Mass'].sum().reset_index(
...
def raddens_post_processing(inputdict):
    ...
    if raddens_data_preparation == 'y':
        ...
        fig, ax = plt.subplots()
        if choice == 'Mass':
            ax.plot(results_rd_df['Bin_center'], results_rd_df['Density [g/cm^3]'], '-',
                    label='Radial density function', color='black')
            ax.set(xlabel='Distance from tube center [Ang]', ylabel='Density [g/cm^3]', title='Radial density function')

        if choice == 'Charge':
            ax.plot(results_rd_df['Bin_center'], results_rd_df['Charge density [e/Ang^3]'], '-',
                    label='Radial density function', color='black')
            ax.set(xlabel='Distance from tube center [Ang]', ylabel='Charge density [e/Ang^3]',
...
def radial_charge_density_analysis(inputdict):
    ...
    # Add all masses of the atoms in each bin to the corresponding bin.
    raddens_df_temp = split_frame.groupby(pd.cut(split_frame['Distance'], raddens_bin_edges))[
        'Charge'].sum().reset_index(name='Weighted_counts')
    raddens_df_temp = pd.DataFrame(raddens_df_temp)
