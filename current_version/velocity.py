...
def velocity_prep(inputdict):
    ...
    velocity_choice = ddict.get_input('Do you want to calculate the velocity of each [1]molecule or each [2]atom? ', args, 'string')
    if velocity_choice == '1':
        inputdict['velocity_choice'] = 'molecule'
        ddict.printLog('Warning: This analysis will yield erroneous results, if the trajectory is wrapped atomwise!\n', color = 'red')
        COM_frame = ut.COM_calculation(first_frame)
...
def velocity_calc_atom(inputdict):
    ...
    all_coords = pd.concat([old_frame[['X', 'Y', 'Z']].add_prefix('old_'), new_frame[['X', 'Y', 'Z']].add_prefix('new_')], axis=1)
...
def velocity_calc_molecule(inputdict): #-> something is still wrong here, the velocity is not calculated correctly probably due too the COM calculation and the wrapping afterwards
    ...
    new_frame = ut.COM_calculation(new_frame)
...
def velocity_analysis_chunk_processing(inputdict):
    ...
    chunk_mean_velocities = [np.mean(grid_points_velocities[i]) if len(grid_points_velocities[i]) != 0 else 0 for i in range(len(grid_points_velocities))]
    ...
    for i in range(len(grid_points_average_velocities)):
        ...
        if total_occurrances == 0:
            ...
            grid_points_average_velocities[i] = (grid_points_average_velocities[i] * grid_points_occurrances[i] + chunk_mean_velocities[i] * chunk_occurances[i]) / (grid_points_occurrances[i] + chunk_occurances[i])
...
def velocity_processing(inputdict):
    ...
    x_vel_profile = np.divide(x_vel_profile, x_count_nonzero, out=np.zeros_like(x_vel_profile), where=x_count_nonzero!=0)
    y_vel_profile = np.divide(y_vel_profile, y_count_nonzero, out=np.zeros_like(y_vel_profile), where=y_count_nonzero!=0)
    z_vel_profile = np.divide(z_vel_profile, z_count_nonzero, out=np.zeros_like(z_vel_profile), where=z_count_nonzero!=0)
...
def rad_velocity_analysis(inputdict):
    ...
    velocity_frame['Distance_bin'] = pd.cut(velocity_frame['Distance'], bins=velocity_bin_edges, labels=velocity_bin_labels)
    ...
    for i in range(num_increments):
        if i in velocity_bin_counter_temp['Distance_bin'].values:
            velocity_bin_counter.loc[i, 'Total count'] += velocity_bin_counter_temp.loc[velocity_bin_counter_temp['Distance_bin'] == i, 'velocity'].values[0]
        else:
    ...
    for i in range(num_increments):
        if i in velocity_df_temp['Bin'].values:
            velocity_df.loc[counter, 'Bin %d' % (i + 1)] = velocity_df_temp.loc[velocity_df_temp['Bin'] == i, 'velocity'].values[0]
        else:
...
def rad_velocity_processing(inputdict):
    ...
    if plot_data == 'y':
        ...
        # Plot
        fig, ax = plt.subplots()
        ax.plot(results_vd_df['Bin_center'], results_vd_df['Average Velocity'], '-', label='Radial velocity profile', color='black')
        ax.set(xlabel='Distance from tube center / $\mathrm{\AA}$', ylabel='Velocity / $\mathrm{\AA/fs}$', title='Radial Velocity Profile')
        ax.grid()
